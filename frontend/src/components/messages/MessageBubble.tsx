...
export function MessageBubble({ message, isOwn, showMeta, isReadByPeer, onEdit, onDelete, onRetry }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex gap-2.5 ${isOwn ? "flex-row-reverse" : ""} ${showMeta ? "mt-3" : "mt-0.5"}`}>
      <div className="w-7 shrink-0">
        {showMeta && !isOwn && <Avatar seed={message.sender_id} name={sender.text} size="sm" />}
      </div>
        ...
        {isEditing ? (
          ...
          >
            {message.has_attachment ? (
              <VoiceMessagePlayer messageId={message.id} durationMs={message.attachment_duration_ms} />
            ) : (
